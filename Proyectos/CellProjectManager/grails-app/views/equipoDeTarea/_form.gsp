...
<div class="fieldcontain ${hasErrors(bean: equipoDeTareaInstance, field: 'tarea', 'error')} required">
	<label for="tarea">
		<g:message code="equipoDeTarea.tarea.label" default="Tarea" />
		<span class="required-indicator">*</span>
	</label>
	<g:select id="tarea" name="tarea.id" from="${business.tarea.Tarea.list()}" optionKey="id" required="" value="${equipoDeTareaInstance?.tarea?.id}" class="many-to-one"/>
</div>
